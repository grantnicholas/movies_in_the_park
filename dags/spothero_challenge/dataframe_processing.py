...
def enrich_movies_with_weather(movies_df, DARKSKY_API_KEY):
    ...
    movies_df["is_forecast_current_year"], movies_df["temperature"], movies_df["humidity"], movies_df[
        "precip_prob"] = zip(
...
def create_final_report(movies_with_weather_df, imdb_df):
    ...
    joined_primary_title_df = movies_with_weather_df.merge(imdb_df, how='left', left_on='title',
                                                           right_on='primaryTitle', validate='many_to_one')
    ...
    joined_original_title_df = not_matched_primary_title_df.drop(columns=imdb_df.columns).merge(imdb_df, how='left',
                                                                                                left_on='title',
    ...
    deviation_cols = ["deviation_average_rating", "deviation_temperature", "deviation_humidity",
                      "deviation_precip_prob"]

    # min-max normalization
    # ensures each feature is on equal footing (scaled from [0,1])
    # otherwise you can't compare temperature in F to humidity in %
    normalized_deviations = (filtered_df[deviation_cols] - filtered_df[deviation_cols].min()) / (
                filtered_df[deviation_cols].max() - filtered_df[deviation_cols].min())

    filtered_df["normalized_deviation_average_ratings"], filtered_df["normalized_deviation_temperature"], filtered_df[
        "normalized_deviation_humidity"], filtered_df["normalized_deviation_precip_prob"] = normalized_deviations[
                                                                                                "deviation_average_rating"], \
    ...
    filtered_df["cost"] = filtered_df["normalized_deviation_average_ratings"] * rating_weight + filtered_df[
        "normalized_deviation_temperature"] * temperature_weight + filtered_df[
                              "normalized_deviation_humidity"] * humidity_weight + filtered_df[
                              "normalized_deviation_precip_prob"] * precip_weight
